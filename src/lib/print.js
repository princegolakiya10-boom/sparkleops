import { fmtLong, fmtDay, todayStr } from './utils';

export function printDailyRunSheet(jobs, dateStr, userName) {
  const date = dateStr || todayStr();
  const dayJobs = jobs.filter(j => j.next === date);
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .date-sub { font-size: 12px; color: #666; margin-top: 2px; text-align: right; }
...
export function exportJobsCSV(jobs) {
  const headers = ['Customer', 'Mobile', 'Address', 'Frequency', 'Next Date', 'Notes'];
  const esc = (s) => `"${String(s || '').replace(/"/g, '""')}"`;
  const rows = jobs.map(j => [esc(j.name), esc(j.mob), esc(j.addr), j.freq, j.next, esc(j.notes)]);
